...
| 2026-09-05 | Ventana de Fabric previa al corte de 0.27.0 (D-57): `fab:proof` sin fallos, 26 hallazgos, 0 sin medir | Capacidad F2 `vergisfablab` | 3 min 06 s encendida (21:10:02→21:13:08 UTC), a US$0,36/h | **US$0,019** | Simón Alero |
| 2026-09-21 | Ventana de Fabric previa al corte de 0.28.0 (D-57): `fab:proof` sin fallos, 26 hallazgos, 0 sin medir | Capacidad F2 `vergisfablab` | ≈ 3 min encendida (~13:55 UTC; sin cronómetro: estimado por la duración del comando, con la pausa en el `trap` y estado final `Paused` verificado), a US$0,36/h | **≈ US$0,02** | Simón Alero |
| 2026-09-21 | Ventana de Fabric previa al corte de 0.29.0 (D-57): `fab:proof` sin fallos, 26 hallazgos, 0 sin medir | Capacidad F2 `vergisfablab` | 2 min 00 s encendida (14:07:43→14:09:43 UTC), a US$0,36/h | **US$0,012** | Simón Alero |
| 2026-09-21 | Ventana de Fabric previa al corte de 0.30.0 (D-57): `fab:proof` sin fallos, 26 hallazgos, 0 sin medir | Capacidad F2 `vergisfablab` | 2 min 35 s encendida (15:24:18→15:26:53 UTC), a US$0,36/h | **US$0,016** | Simón Alero |
| 2026-09-21 | Ventana de Fabric previa al corte de 0.31.0 (D-57): `fab:proof` sin fallos, 26 hallazgos, 0 sin medir. **Dos tramos, y el primero no midió**: `FAB_SERVER` no estaba documentado (el README lo remitía a un `RESOURCES.md` inexistente) y el descubrimiento del endpoint **exige la capacidad prendida** — con `Paused` el API de Fabric devuelve `InternalServerError`, que parece falla del servicio y es estado del recurso. El primer tramo se abortó sin endpoint; el segundo lo descubrió al tercer reintento (~2 min tras marcar `Active`) y corrió el arnés completo. Se asienta el total, no solo el tramo que sirvió. El endpoint y el procedimiento quedaron escritos en `scripts/README-fabric-lab.md` para que no se repita | Capacidad F2 `vergisfablab` | 6 min 18 s encendida sumando los dos tramos (16:56:56→16:58:22 y 16:59:42→17:04:34 UTC), a US$0,36/h | **US$0,038** | Simón Alero |
| 2026-09-21 | **El peldaño que la doctrina de `UNMASK` nunca corrió** (local/unmask-grantee-nombrado.ts): `GRANT UNMASK ON [dbo].[areas]([rut]) TO [vergis-lab-serving-sp]` **acepta y surte efecto sin `CREATE USER`**, con premisa verificada (el SP leía `xxxx`), efecto medido en conexión nueva (`33.333.333-3`) y **control negativo** (tras el `REVOKE` volvió a `xxxx` ⇒ el GRANT era la causa). Antes se inferían las dos cosas del rechazo de `CREATE USER` y se publicó «no se puede afinar más (medido)»; el arnés viejo declaraba en su salida «E3 queda SIN MEDIR». `fab:proof` en la misma ventana: sin fallos, 26 hallazgos, 0 sin medir | Capacidad F2 `vergisfablab` | 5 min 06 s encendida (01:10:48→01:15:54 UTC), a US$0,36/h | **US$0,031** | Simón Alero |
| 2026-09-23 | Ventana de Fabric previa al corte de 0.34.0 (D-57): `fab:proof` sin fallos, 26 hallazgos, 0 sin medir. **Dos tramos, y el primero no midió C2**: la credencial del segundo principal (`FAB_CONSOLA_SP_*`) no está en `local/`; el primer tramo salió con código 3 (1 sin medir). El segundo corrió C2 con el SP de laboratorio `vergis-lab-serving-sp` (rol `Viewer`, sin `UNMASK`: lee `xxxx` en P10) como segundo principal: control positivo, re-set rechazado, falla por 15664. Se asienta el total | Capacidad F2 `vergisfablab` | ≈ 7 min 52 s encendida (11:13:35→11:17:27 y 11:20:33→11:24:33 UTC, cronometrado por el comando, pausa en el `trap` y `Paused` verificado), a US$0,36/h | **≈ US$0,047** | Simón Alero |
| 2026-09-23 | Ventana de Fabric previa al corte de 0.35.0 (D-57): `fab:proof` sin fallos, 26 hallazgos, 0 sin medir, en un solo tramo; C2 con el SP de laboratorio `vergis-lab-serving-sp` (rol `Viewer`) como segundo principal | Capacidad F2 `vergisfablab` | 2 min 56 s encendida (17:02:03→17:04:59 UTC, resume `--no-wait` + sondeo de estado, pausa en el `trap` y `Paused` verificado), a US$0,36/h | **≈ US$0,018** | Simón Alero |
| 2026-09-23 | Ventana de Fabric previa al corte de 0.35.1 (D-57): `fab:proof` sin fallos, 26 hallazgos, **1 sin medir** (C2). **Tres tramos, ninguno midió C2**: el 1.º y el 3.º (este con 90 s de espera tras `Active`) dieron «CONTROL POSITIVO FALLIDO (operation timed out)» en C2; el 2.º no conectó al endpoint en 60 s (exit 2). Se asienta el total | Capacidad F2 `vergisfablab` | ≈ 17 min 39 s encendida (18:42:32→18:49:25, 18:49:35→18:54:57 y 18:55:06→19:00:30 UTC, pausa en el `trap` y `Paused` verificado en cada tramo), a US$0,36/h | **≈ US$0,106** | Simón Alero |
| 2026-09-23 | Ventana de Fabric previa al corte de 0.36.0 (D-57): `fab:proof` sin fallos, 26 hallazgos, 0 sin medir, en un solo tramo; C2 con el SP de laboratorio `vergis-lab-serving-sp` (rol `Viewer`) como segundo principal | Capacidad F2 `vergisfablab` | 4 min 43 s encendida (22:10:52→22:15:35 UTC, resume `--no-wait` + 90 s de calentamiento, pausa en el `trap` y `Paused` verificado), a US$0,36/h | **≈ US$0,028** | Simón Alero |
