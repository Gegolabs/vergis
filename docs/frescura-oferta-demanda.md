...
| **Lazo de frescura** (observar → alertar → reconciliar) | ✅ `server/freshness-loop.ts`, cada `VERGIS_FRESHNESS_POLL_MS` (default 5 min; `0` lo apaga). Los avisos se gatean con los destinos declarados en `VERGIS_NOTIFY`; la observación y el reconcile corren igual |
| Observabilidad — **aviso autónomo** (push ante fallida/faltante) | ✅ fase 2 del lazo; dedup por transición (`freshnessAlerts`/`diffAlertState`), estado persistido en `platform_setting`. Con el motor caído clasifica sobre lo proyectado: un motor sin respuesta no es un proceso atrasado. El destino es declarativo y N simultáneos (`VERGIS_NOTIFY` → Slack o webhook genérico, `server/notify.ts`): el producto compone un mensaje canal-agnóstico con dominio, proceso, motivo, hora esperada y enlaces profundos a la corrida y a la frescura del dominio (`VERGIS_PUBLIC_URL`, requerida si hay destinos) |
