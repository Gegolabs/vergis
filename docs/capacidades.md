...
| `CAP-15` | Override por columna de las afordancias automáticas | `sortable`, `searchable`, `filter` (booleano; string `vals \| num \| date` → CAP-191), `groupBy`, `colorscale` | 0.18.0 | [catalogo-elementos.md §4·bis](catalogo-elementos.md) |
...
| `CAP-192` | **Clase de embudo declarada por columna** (`filter: vals \| num \| date`), prevalece sobre la heurística del dato — la salida para los identificadores numéricos (`Id Persona`, folio, número de documento), a los que «Positivos / Negativos / En cero» no les dice nada | `columns[].filter: vals \| num \| date` (el booleano conserva su significado de override del auto-on) | 0.29.0 | [catalogo-elementos.md §4·ter](catalogo-elementos.md) |
| `CAP-193` | **Subtotal por grupo en la fila de cabecera de cada nivel de agrupación** (columnas con `total`), mismo cálculo que el pie y sigue a los filtros | `table.columns[].total` + agrupación del usuario en la bandeja (opt-in doble; sin vocabulario nuevo en el spec) | `0.29.0` | [catalogo-elementos.md §4·quinquies](catalogo-elementos.md) |
...
| `CAP-198` | **Consola SQL: T-SQL libre sobre un Conector registrado, acotado por la misma RLS que un PI.** Ejecuta bajo un **principal de consola** propio del Conector (jamás el de serving, que es Admin), con los claims del ingeniero en `SESSION_CONTEXT` marcado `@read_only` y una conexión dedicada que se cierra. Un Conector se ofrece **solo si se mide** que su principal no puede escribir, que toda tabla base tiene política nativa —**medido bajo el principal de serving**, porque el gobierno es propiedad del terreno y `sys.security_policies` está filtrada por permiso (#340); si el principal que sondea no puede ver la vista, el Conector se rechaza por «no se pudo medir el gobierno», que NO es «no hay política»— y que el motor honra `@read_only`. Una tabla base cuenta como **cubierta** por `SECURITY POLICY` **o** porque el principal de consola no tiene `SELECT` sobre ella (`DENY SELECT ON <tabla> TO [<principal de consola>]`, medido con `HAS_PERMS_BY_NAME`): lo que no se puede leer no puede filtrar, y el `DENY` es de objeto y de un principal, así que no toca al pipeline como sí lo haría una política de fila `deny` (#342). Un permiso que no se pudo medir NO cubre, y las tablas excluidas por permiso no se listan en el árbol de esquema; un Conector con reglas de columna **no se ofrece** (bajo SQL libre el predicado se evalúa contra el valor real: la máscara se infiere sin verse). Toda ejecución deja entrada en un log append-only con el actor del gate | `VERGIS_CONSOLA_ENABLED` · `VERGIS_CONSOLA_SCOPE_GROUP` · `VERGIS_CONSOLA_MAX_ROWS` · `VERGIS_CONSOLA_TIMEOUT_MS` · `VERGIS_CONSOLA_MAX_CONCURRENTES`; sub-perfil `consola` en `VERGIS_CONNECTIONS`; `GET /consola`; `/contrato.consola` | 0.33.0 (#306) | [consola-sql.md](consola-sql.md) |
...
| `CAP-186` | Store de **evaluaciones** embebido y opt-in (instrumento inmutable por id · intento · respuesta verbatim con confianza derivada · revisión · reporte), sin superficie HTTP propia | `VERGIS_EVALUACIONES=1` / `VERGIS_EVALUACIONES_DB`; label `vergis.schema.stores` (`evaluaciones=1`); `control.store[]` de `/contrato` cuando está encendido | 0.27.0 | [CHANGELOG · Sin publicar](../CHANGELOG.md) |
| `CAP-187` | **Segundo proto-Botlet: `daftar`** — el nodo hospeda un Let evaluador (catálogo por identidad, aplicación del instrumento, corrección, reporte e impresión) además de los PIs de Mira | spec `daftar.yaml` con clave raíz `daftar_version` en `VERGIS_SPECS_DIR`; instrumentos en `VERGIS_INSTRUMENTOS_DIR`; store con `VERGIS_EVALUACIONES=1`; claim `student` en el directorio de identidad | 0.27.0 | [CHANGELOG · Sin publicar](../CHANGELOG.md) |
| `CAP-188` | **Puerta de salida genérica del nodo** (`ProtoBotlet.invoke`): el router despacha `/<slug>[/…]` por la familia del Let, y Mira pasa por ella sin cambiar de conducta | interno del nodo — no se declara en ningún spec; el `Report.proto` decide quién atiende | 0.27.0 | [CHANGELOG · Sin publicar](../CHANGELOG.md) |
| `CAP-189` | **Arranque sin motor de datos**: un nodo cuyas specs no consumen datos gobernados no exige datasets, conexiones ni bootstrap del esquema | derivado del catálogo (`consumesData` del proto), no de una env | 0.27.0 | [CHANGELOG · Sin publicar](../CHANGELOG.md) |
...
| `CAP-199` | **Código estable del desenlace** en el contrato `_logs/`: sufijo opcional al final de la línea, con los datos del caso; se persiste con el desenlace y el motivo queda sin él. Un sufijo que no calza entero no existe | `… — <motivo> ⟦<familia>[/<especifico>] clave=valor …⟧` · `FileOutcome.codigo`/`params` · columnas `desenlace_codigo`/`desenlace_params` | 0.34.0 (#346) | [contrato-ingesta-logs.md §2](contrato-ingesta-logs.md) |
| `CAP-200` | **Catálogo de guías de carga**: 14 familias del Producto con actor fijo (usuario · operador · nadie) y guía genérica, más familias y guías propias de la instancia, por código y opcionalmente por slot; validado al arrancar y recargado en caliente conservando lo vigente si viene roto | bloque raíz `guias:` (`familias`, `entradas`) del archivo `VERGIS_INTAKE` · `resolverGuia` | 0.34.0 (#346) | [contrato-ingesta-logs.md §7](contrato-ingesta-logs.md) |
| `CAP-201` | **La guía se le muestra a quien subió**: celda Desenlace con actor, qué pasó y qué hacer (motivo técnico completo plegado), página «Errores frecuentes» por casilla ordenada por frecuencia, correo con la guía y señal de cobertura para el operador | `/admin/dominio/<id>/errores/<slot>` (gate de Cargas) · flujo `cargas-usuario` | 0.34.0 (#346) | [contrato-ingesta-logs.md §7](contrato-ingesta-logs.md) |
| `CAP-202` | **El estado de una carga sale solo de lo declarado y lo registrado, y avanza hasta uno final** con historia de intentos: lo que el job declaró de ESE archivo (por su nombre conocido) más los actos registrados —re-subida, retiro en `_retirado/` en sus tres formas, reversión, «Deshacer»—; la ausencia de declaración nunca es un resultado; un único puente histórico anclado a `contrato_desde` por slot | `resolverEstadoDeCarga` · estados finales `procesada` · `retirada` · `reemplazada` · `deshecha` · tabla `intake_intento` · claves de slot `contrato_desde`, `target.processed` | 0.35.0 (#351) | [contrato-ingesta-logs.md §3](contrato-ingesta-logs.md) |
| `CAP-203` | **Consola de Cargas veraz**: «Recibido» y «⏳ Cargando» en vez de celda vacía, los estados finales con su fecha, el motivo del rechazo en la puerta, qué tomó cada corrida de ese tipo de archivo, «Vigente» para el slot que no archiva, **señal de contrato** (lo que salió del landing fuera de `_retirado/`, lo declarado «no cargado» y archivado igual, nombres que calzan con dos tipos) y aviso de duplicado que cita la carga más reciente con su estado | `/admin/dominio/<id>/cargas?slot=<slotId>` · puerta con nombre normalizado a NFC y rechazo del nombre que calza con dos o más tipos | 0.35.0 (#351) | [contrato-ingesta-logs.md §4](contrato-ingesta-logs.md) |
| `CAP-204` | **Aviso al operador por carga** `sin-informe` o con guía de actor operador, deduplicado; el aviso al usuario nombra a quién avisar según lo que la instancia declaró (destino suscrito, o `contacto`, o nada) | flujo `cargas-operador` en `VERGIS_NOTIFY` (opcional) · clave `contacto:` del archivo de intake (raíz, heredable por slot) | 0.35.0 (#351) | [contrato-ingesta-logs.md §5](contrato-ingesta-logs.md) |
| `CAP-205` | **Puerta única de carga con ficha por tipo de archivo**: zona general que enruta cada archivo por su nombre (lo inequívoco va solo; lo ambiguo, o lo que solo un tipo sin patrón aceptaría, se manda a elegir; nunca más estricta que la subida con tipo elegido), revisión en el navegador antes de subir, y una página por tipo con la ficha que declara la instancia (qué es, de dónde se saca, cómo se llama, qué reemplaza, qué va junto, qué subir antes), sus cargas en palabras de quien sube y sus problemas frecuentes | `/cargar` · `/cargar/<slot>` · bloque `ficha:` del slot (`origen`, `nombre`, `ejemplo`, `regimen`, `clave`, `juego`, `requiere`, `cadencia`) · `meta[].ayuda` · `describirPatron` | 0.36.0 | [gestion-de-dominio.md §4](gestion-de-dominio.md) |
| `CAP-206` | **Estado vivo de las cargas sin empujar nada**: el fragmento de la lista se pide cada 10 s mientras haya cargas no finales (pausa con la pestaña oculta) y lee solo el registro y la proyección; tras subir o retirar, el vigilante observa y resuelve ese tipo cada 30 s mientras tenga cargas no finales de menos de 45 min | `GET /cargar/<slot>/cargas` (cabecera `x-hay-no-finales`) · `intakeLoop.acelerar` | 0.36.0 | [gestion-de-dominio.md §4](gestion-de-dominio.md) |
| `CAP-207` | **Los flujos de aviso en el contrato operativo**: cuántos destinos tiene suscritos cada flujo ahora mismo, sin direcciones | `/contrato` → `avisos` (`alerts`, `reports`, `cargas-usuario`, `cargas-operador`) | 0.36.0 | [contrato-ingesta-logs.md §4](contrato-ingesta-logs.md) |
...
| `CAP-161` | Cotejo del **catálogo de capacidades** contra lo declarado en máquina | `npm run capacidades:cotejo` | 0.23.0 | [§¿Qué garantiza el cotejo?](#qué-garantiza-el-cotejo) |
...
| `CAP-165` | Config recargable por slice, y una señal que fuerza la recarga sin cortar | `VERGIS_NOTIFY` · `VERGIS_PI_OWNERS` · `VERGIS_SOURCES` (y `VERGIS_MENU` desde CAP-194, `VERGIS_STATIC` desde CAP-195); `SIGHUP` | 0.15.0 | [arquitectura-multi-reporte.md §6](arquitectura-multi-reporte.md) |
...
| `CAP-195` | **El nodo sirve las colecciones de archivos estáticos declaradas por la instancia**, bajo la misma autorización del catálogo: publicar una página deja de exigir un bloque en el borde. Prefijo validado al cargar (un Let con el mismo slug gana), contención de ruta verificada léxica y realmente (`..`, `%2e%2e%2f` y symlink que escapa ⇒ 403), `Content-Type` por lista blanca + `nosniff`, lectura por request (sin caché) y recarga en caliente | `VERGIS_STATIC` → `static: [{ path, dir, label? }]`; entra en `RELOADABLE_SLICES` y `GET /contrato` la declara con `exists`/`readable`/`shadowedByLet` | 0.31.0 | [arquitectura-multi-reporte.md §Config declarativa](arquitectura-multi-reporte.md) |
| `CAP-196` | **El nodo publica sus novedades**: el `CHANGELOG.md` que viaja dentro de la imagen, servido como página con el marco de la plataforma y **la misma autorización que el resto del nodo** (sin rol nuevo). La versión que corre encabeza, el historial va debajo con anclas estables, y la sección «Sin publicar» **no se sirve**. El número de versión del pie del inspector y del catálogo enlaza a la ruta | `GET /novedades` · `/novedades#<versión>`; sin env nueva (503 si el changelog no viajó en la imagen) | 0.32.0 | [CHANGELOG · Sin publicar](../CHANGELOG.md) |
| `CAP-197` | **El nodo GENERA y sirve el Datadoc**: el catálogo del esquema de datos de la instancia —qué existe en cada conexión, qué significa, quién lo escribe, quién lo lee, con qué frescura y bajo qué gobierno—, medido por conexión en modo estrictamente de solo lectura, cacheado en disco con sello por conexión, con builds inmutables y puntero atómico, y servido en `/datadoc/` por la misma vía que `CAP-195`. Una conexión que no responde conserva su medición anterior y el sitio la marca. **Los conteos de filas solo se piden donde la RLS no filtra** (clasificación por la forma COMPLETA del predicado, jamás por substring), y un build marcado rancio por una recarga de gobierno los retira | `VERGIS_DATADOC=1` (exige `VERGIS_ENGINE=fabric`) · `VERGIS_WRITERS` · `VERGIS_SEMANTICA` (ambos en `RELOADABLE_SLICES`) · `domains[].connections` · `GET /datadoc/` · `/admin/datadoc` (estado, generar, schedule y política de conteos) · lazo `datadoc` · sección en `GET /contrato` | 0.33.0 | [arquitectura-multi-reporte.md §El nodo genera el catálogo del esquema](arquitectura-multi-reporte.md) |
