...
| `CAP-186` | Store de **evaluaciones** embebido y opt-in (instrumento inmutable por id · intento · respuesta verbatim con confianza derivada · revisión · reporte), sin superficie HTTP propia | `VERGIS_EVALUACIONES=1` / `VERGIS_EVALUACIONES_DB`; label `vergis.schema.stores` (`evaluaciones=1`); `control.store[]` de `/contrato` cuando está encendido | sin publicar (#291) | [CHANGELOG · Sin publicar](../CHANGELOG.md) |
| `CAP-187` | **Segundo proto-Botlet: `daftar`** — el nodo hospeda un Let evaluador (catálogo por identidad, aplicación del instrumento, corrección, reporte e impresión) además de los PIs de Mira | spec `daftar.yaml` con clave raíz `daftar_version` en `VERGIS_SPECS_DIR`; instrumentos en `VERGIS_INSTRUMENTOS_DIR`; store con `VERGIS_EVALUACIONES=1`; claim `student` en el directorio de identidad | sin publicar (#295) | [CHANGELOG · Sin publicar](../CHANGELOG.md) |
| `CAP-188` | **Puerta de salida genérica del nodo** (`ProtoBotlet.invoke`): el router despacha `/<slug>[/…]` por la familia del Let, y Mira pasa por ella sin cambiar de conducta | interno del nodo — no se declara en ningún spec; el `Report.proto` decide quién atiende | sin publicar (#295) | [CHANGELOG · Sin publicar](../CHANGELOG.md) |
| `CAP-189` | **Arranque sin motor de datos**: un nodo cuyas specs no consumen datos gobernados no exige datasets, conexiones ni bootstrap del esquema | derivado del catálogo (`consumesData` del proto), no de una env | sin publicar (#295) | [CHANGELOG · Sin publicar](../CHANGELOG.md) |
