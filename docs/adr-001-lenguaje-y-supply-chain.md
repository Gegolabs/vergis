...
Donde un bug de lógica sí sería grave — el compilador de policies — la garantía no proviene del type system sino del diseño: IR no-Turing-completo, evaluador de referencia como oráculo, y 2.100 iteraciones de property testing diferencial que prueban `filas(ClickHouse) ≡ filas(Fabric) ≡ filas(referencia)`.
...
| Cooldown de actualizaciones | Renovate con `minimumReleaseAge` de 14 días — los compromisos de paquetes npm se detectan típicamente en horas/días |
