...
> qué se ve en la **cara**, qué vive en la **bandeja** (Inspector) y qué se lleva el **print**.
...
**cara = estado · bandeja = maquinaria · print = estado como texto.**
...
- La **bandeja** (Inspector) guarda la **maquinaria**: los pickers de filtro, la búsqueda, el
...
| Superficie | **sello** siempre visible y clickeable en la banda | **maquinaria** en la bandeja; su estado activo, como chip en la cara |
...
sello **es** el control (una cosa, un lugar — no se duplica en la bandeja):
...
- **single** → un `<select>` nativo estilizado como sello (accesible por teclado sin reinventar el
  ...
  contexto — la MISMA mecánica del control histórico de la bandeja.
