 ...
 *
 * El segundo handle se abre en modo `read` (handle de inspección): con el store escritor vivo, el único
 * handle admisible sobre el archivo es uno que no escriba — un segundo escritor sería justo lo que el
 * fencing de `sqlite.ts` existe para delatar.
 */
...
describe('GovernanceStore · ledger de publicaciones de jobs', () => {
  it('la publicación queda en el ARCHIVO apenas se registra (sin cerrar el store)', async () => {
    ...
    // Segunda apertura del MISMO archivo (inspección), con g1 todavía vivo: lo que se lee es lo
    // volcado por la escritura, no por el cierre.
    const g2 = await SqliteGovernanceStore.open(file, {}, { mode: 'read' })
