...
2. **La RLS del motor hace el trabajo, igual que para un PI.** El ingeniero ve las filas que su claim autoriza y las columnas con regla de columna **siempre enmascaradas**. ⚠️ **Y la promesa «ve exactamente lo que un PI le mostraría» es FALSA bajo DDM con SQL libre, aun con la máscara honrada** (hallado en la segunda mirada del 2026-09-21): un PI tiene consultas fijas; la Consola tiene `WHERE` libre, y con predicados sobre una columna enmascarada (`BETWEEN`, `LIKE`) se **infiere** el valor sin verlo. La documentación de Fabric lo declara así, y el arnés del Producto prueba nueve formas de proyección y cómputo pero **ninguna de inferencia por predicado**. Esto no lo arregla ninguna doctrina de `UNMASK`: es un límite de DDM. Dos salidas, y la superficie tiene que elegir una explícitamente — declararlo en la Consola, o endurecer la condición (c) a «no ofrecible si hay reglas de columna, punto». El experimento **C3** lo mide. Eso es *menos* de lo que un ingeniero espera de una consola — y es correcto: la Consola de Mira sirve para explorar *lo que un PI mostraría*, no para administrar la fuente (para eso está el SQL endpoint de Fabric bajo la identidad propia del ingeniero, con su auditoría propia).
...
| **C3** | **Inferencia por predicado bajo DDM**: con la máscara honrada, correr `SELECT COUNT(*) FROM t WHERE <col enmascarada> BETWEEN a AND b` y `… LIKE '33.%'` variando los rangos | alguna forma permite acotar el valor real | Ninguna: el motor rechaza o el resultado no discrimina. **Si el motor responde, el hallazgo es que DDM no protege bajo SQL libre** y la condición (c) se endurece a «no ofrecible si hay reglas de columna, punto» |
