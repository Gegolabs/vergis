...
**Solo `engine=fabric`.** En ClickHouse los claims viajan como **settings de request** por query-param HTTP (`execute-sql-ch.ts:56-60`), y una `SELECT … SETTINGS vergis_claim_groups='x'` del usuario compite por **el mismo canal**: el motor no distingue el setting que puso el nodo del que puso el texto. `readonly=1` bloquea *todo* cambio de settings — incluidos los del nodo por URL — y `readonly=2` los permite a ambos. **Se asume** (no verificado) que un `SETTINGS PROFILE` con `CHANGEABLE_IN_READONLY` tampoco separa los dos orígenes. Hasta que exista un experimento que demuestre un canal exclusivo del nodo (p. ej. usuario efímero por request, o `SET ROLE` + row policy por rol), ofrecer la Consola sobre ClickHouse es un bypass de fila. Queda **fuera de alcance con esta razón** en `docs/consola-sql.md` y en `/contrato` (`consola.motor: 'fabric'`; con `engine=clickhouse` la capacidad reporta `disabledReason`).
...
|--|--|--|--|
| **P-1** | ¿Existe una política de **escritura** ad-hoc (el «CRUDLEX» del issue)? Hoy el policy store no la conoce | v1 solo lectura; diseñar escritura como issue aparte con su propia doctrina (confirmación, transacción, log con sentencia) | Nada de v1 |
...
| **P-6** | ¿Consola sobre ClickHouse? | Fuera hasta demostrar un canal exclusivo del nodo (§10) | Nada de v1 |
