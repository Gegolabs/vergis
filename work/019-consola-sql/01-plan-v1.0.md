...
| Versión de este plan | 1.6 · el recorte del gate (c) desaparece: el grantee acotado de `UNMASK` está MEDIDO (`4d9b5e8`, `docs/gobierno-permisos.md`), la doctrina ya no concede a `public` y P-4 cierra; C3 (inferencia por predicado) queda separado de (c) como problema propio, con default fail-closed y P-7; entra el límite de César «una consulta a la vez» (ficha 02) como requisito, y el instrumento de fallos por capacidad como gate de la Fase 2; el costo del login pasa a C10 · 1.5 · la promesa «ves lo que un PI te mostraría» es falsa bajo DDM con SQL libre (inferencia por predicado): entra C3 y la condición (c) puede tener que endurecerse · 1.4 · P-3 despachada (XLSX: escritor propio client-side, sin dependencia) · 1.3 · P-2 CERRADA por César: la Consola no se restringe; queda la verificación de las tablas `grant: all` antes de abrir · 1.2 · P-2 reencuadrada (el permiso de superficie no protege el dato; lo que acota es el alcance entre tablas) · 1.1 · 2026-09-21 — cierra la segunda mirada: P-1 y P-6 resueltas, ClickHouse sale por alcance y no por límite del motor (medido), y la sonda del gate (d) prueba con la clave real en el mismo batch |
...
2. **La RLS del motor hace el trabajo, igual que para un PI.** El ingeniero ve las filas que su claim autoriza y las columnas con regla de columna **siempre enmascaradas**. Para las **filas** la equivalencia con el PI es exacta: la misma `SECURITY POLICY` sobre el mismo `SESSION_CONTEXT`. Para las **columnas enmascaradas** hay dos problemas distintos, y el plan los trata por separado porque tienen causas y remedios distintos:
   - **Herencia de `UNMASK` — condición (c) del gate (§6).** Que el principal de consola **no** pueda desenmascarar. La doctrina de `docs/gobierno-permisos.md` concede `UNMASK` **al principal de serving nombrado** (`GRANT UNMASK ON [tabla]([col]) TO [<principal de serving>]` — medido el 2026-09-21: acepta y surte efecto sin `CREATE USER`, con control negativo), así que el principal de consola no lo hereda y (c) pasa. Se **mide** igual, por Conector, con el centinela.
   - **Inferencia por predicado — experimento C3 (§14).** Aun con la máscara honrada, SQL libre permite `WHERE <col enmascarada> BETWEEN a AND b` o `LIKE '33.%'` y **acotar** el valor sin verlo; un PI no lo permite porque sus consultas son fijas. Esto es un límite de DDM que **ninguna doctrina de `UNMASK` toca**: la medición del grantee no lo resuelve ni lo empeora. La documentación de Microsoft lo declara, y el arnés del Producto prueba nueve formas de proyección y cómputo pero **ninguna de inferencia por predicado**. C3 lo mide **primero y gratis** en el arnés T-SQL local; su resultado decide si un Conector con reglas de columna es ofrecible (default fail-closed, P-7).

   Eso es *menos* de lo que un ingeniero espera de una consola — y es correcto: la Consola de Mira sirve para explorar *lo que un PI mostraría*, no para administrar la fuente (para eso está el SQL endpoint de Fabric bajo la identidad propia del ingeniero, con su auditoría propia).
...
4. **Un Conector se ofrece en la Consola solo si su principal de consola no puede escribir, no puede desenmascarar y toda tabla que puede leer tiene política nativa.** Un warehouse con una tabla sin `SECURITY POLICY` no se ofrece entero: la Consola no puede acotar por tabla sin un parser (§5), y una tabla sin política devuelve todas sus filas (`server/sql-tables.ts:1-4`). Acotarlo por objeto con `GRANT`/`DENY` al principal de consola nombrado **no está medido** (§6) y no se supone.
...
| Plano de columna | `fabric.ts:255-300` (DDM) y `:370-400` (vista de máscara) | Dos mecanismos: DDM en la columna (discrimina por permiso `UNMASK` del **principal**) + vista que discrimina por `SESSION_CONTEXT`. La vista **solo discrimina si el principal tiene `UNMASK`**; sin `UNMASK`, base y vista salen enmascaradas (sobre-enmascaramiento, nunca fuga). Doctrina vigente (`docs/gobierno-permisos.md` §«Cómo se concede la capacidad», medida el 2026-09-21, commit `4d9b5e8`): `GRANT UNMASK ON [tabla]([col]) TO [<principal de serving>]` — **al principal nombrado por su display name**: el `GRANT` acepta y surte efecto **sin `CREATE USER`** (el propio `GRANT` crea el usuario de base), con premisa verificada y control negativo por `REVOKE`. `TO [public]` queda como último recurso con su costo dicho (alcanza a todo principal del warehouse). Sin medir, con esas palabras: si el `appId` sirve igual que el display name, y si el SKU de un cliente se comporta igual (se midió en el F2 propio con datos sintéticos). |
...
`sp_set_session_context … @read_only = 1`: a partir de ahí, un nuevo `sp_set_session_context` sobre la misma clave **falla** en el motor (SQL Server: error 15664, «Cannot set key … The key has been set as read_only for this session»). **No verificado en Fabric Warehouse ni en SQL endpoint de lakehouse** — es el experimento **C2** (§14): si Fabric ignorara `@read_only`, el diseño de fila cae y la Consola **no se implementa** hasta resolverlo. Y como `read_only` deja la clave fija por toda la sesión, la conexión **no puede volver a un pool**: se cierra (`pool.close()` de un `ConnectionPool` con `max: 1` creado por ejecución). El costo es un login AAD + TLS por ejecución; el token viene del caché del `CredentialProvider` (`aad-token.ts:106-122`), así que es un handshake, no una adquisición — se mide en **C10**.

### ¿Qué ve entonces el ingeniero?

Sus filas —exactamente las que vería en un PI **para su identidad**— y las columnas con regla siempre enmascaradas (§6); la equivalencia con el PI es exacta para filas y, para columnas enmascaradas, depende de lo que C3 mida (§14). Menos de lo que espera de una consola de administración — **y se le dice en la propia superficie** («Esta consola muestra lo que Mira mostraría *a ti*. Para administrar la fuente usa el SQL endpoint de Fabric con tu cuenta.»).
...
El PI pasa por un gate por tabla (`sql-tables.ts` + `verifyFabricServability`): toda tabla que toca debe tener `SECURITY POLICY`. La Consola no puede hacerlo por tabla sin un parser (§5), así que la unidad de decisión honesta es **el Conector entero**. Que Fabric acepte `GRANT`/`DENY` **por objeto** al principal de consola nombrado es plausible —el `GRANT UNMASK` por columna a un principal nombrado está medido (`docs/gobierno-permisos.md`, 2026-09-21)— pero **no está medido para `SELECT` por tabla**, y este diseño no depende de ello: un gate por Conector es correcto con o sin esa capacidad. Si algún día se mide y funciona, es una v1.x que refina la unidad, no un cambio de doctrina.
...
**Consecuencia declarada para columnas:** en la Consola las columnas con regla salen **siempre enmascaradas** (default de DDM: `0`/`xxxx`), aunque el claim del ingeniero las desenmascararía en el PI a través de la vista. Es sobre-enmascaramiento en la dirección segura y va dicho en la superficie. **No se refina, y la razón es del diseño, no de Fabric:** la vista solo discrimina por claim si el principal tiene `UNMASK`, y un principal de consola con `UNMASK` lee la **tabla base** en claro bajo SQL libre — o sea, viola (c). Con SQL libre no hay punto intermedio entre «siempre enmascarada» y «fuga». **C6** lo verifica en los dos motores (base y vista salen enmascaradas bajo el principal de consola).

**Lo que (c) NO cubre, y va dicho para que nadie lo confunda:** (c) garantiza que la máscara se **honra**; no garantiza que el valor no se **infiera** con predicados sobre la columna enmascarada (`BETWEEN`, `LIKE`). Eso es **C3** (§14), un límite de DDM independiente de a quién se concede `UNMASK`. Mientras C3 no esté medido —y por default si C3 refuta— **un Conector con `columnRules` no se ofrece, punto** (P-7): es la única lectura consistente con «no ofrecer con advertencia». Bajo ese default (c) **se sigue midiendo y reportando** en `/contrato` (es la condición que vuelve a ser la operativa cuando P-7 se relaje), pero el motivo publicado es el de P-7, que es el que manda. Hoy cuesta cero: ninguno de los ocho almacenes de la instancia de referencia tiene reglas de columna (informado por la ficha `lab/work/255/03`, verificado ahí sobre las políticas y los nueve specs; **no re-verificado en este plan**).
...
**Visibilidad por Conector — DECIDIDO el 2026-09-21 (César): no se restringe.** Se ofrecen todos los Conectores que pasan el gate, y lo que cada uno muestra lo decide la RLS. Un recorte de superficie no protege el dato —lo hacen (a)(b)(c)(d)— y tampoco es control de acceso: si la autorización se ancla en el dato, **la política es la autoridad, no el PI**; que un ingeniero vea las filas que su claim autoriza en una tabla sin PI no es fuga, es la política cumpliéndose. El árbol de esquema tampoco expone nada nuevo: el Datadoc ya publica el catálogo a todo autenticado, con la misma autenticación. ⚠️ **Lo que sí se hace antes de abrir, y es verificación y no restricción:** listar las tablas con `grant: all` por Conector y revisarlas una por una. Esas políticas se escribieron pensando en que las leerían **PIs**; una apertura explícita hecha para un PI pasa a ser legible por cualquiera que la Consola alcance, y conviene saber si alguna quedó abierta por comodidad en vez de por decisión. El gate de I5 **no** depende de ningún permiso de superficie.
...
| **Concurrencia global — UNA consulta a la vez** | `VERGIS_CONSOLA_MAX_CONCURRENTES` | **`1`** | contador en el handler | `503` «consola ocupada: hay una consulta en curso de otra persona» (sin decir de quién) |

**«Una consulta a la vez» es requisito, no ajuste** (César, ficha de despacho `lab/work/255/02`, 2026-09-21: la Consola corre contra la misma capacidad F2 de producción de la que cuelgan los PI, los warehouses y las ingestas, y **compite con lo que está sirviendo**; la capacidad se paga por tiempo encendida, así que el problema es servicio, no factura). El default es `1` y el ejecutor lo implementa con test (segunda ejecución de **otra** identidad ⇒ `503` mientras la primera está en vuelo; al terminar la primera, la segunda entra). Que el env exista es para que una instancia con capacidad holgada pueda subirlo **por decisión del operador**; en la instancia de referencia se queda en `1` (§17). El límite por identidad se conserva porque es lo que da sentido a `cancelar`.
...
- **XLSX**: el issue lo pide; **#61 (César, 2026-07-13) lo descartó** («habría exigido la primera dependencia gorda del producto y un endpoint server-side, sin demanda que lo justifique»). Es una decisión vigente que este issue **reabre**, y no se contradice por cuenta propia:
  ...
  → **DESPACHADA por César el 2026-09-21: opción B**, escritor XLSX propio client-side, sin dependencia. Su decisión de #61 descartaba el `.xlsx` por el **costo** (dependencia gorda + endpoint server-side), no por el formato, y esta vía no paga ninguno de los dos. ⚠️ Que el escritor mínimo sea barato es **conjetura sin medir**, y él marcó la opción sabiéndolo: el ejecutor lo mide antes de comprometer alcance, y si no resulta barato vuelve al despacho — no se convierte en dependencia por inercia.
...
| `VERGIS_CONSOLA_MAX_CONCURRENTES` | `1` | §8 — **una consulta a la vez** por decisión de César; subirlo es del operador de la instancia |
...
"consola": {
  ...
  "limites": { "timeoutMs": 60000, "maxRows": 5000, "maxConcurrentes": 1 },
...
- **Problema**: sin esto, un Conector con principal escritor, tabla sin política o un principal de consola que herede `UNMASK` (p. ej. una instancia que todavía concede `TO [public]`) sería un bypass.
...
- **Tests** (`tests/consola-gate.test.ts`, puros con ejecutor fake): permiso `INSERT` ⇒ no ofrecible con motivo que lo nombra · tabla sin política ⇒ motivo la nombra · vista sobre tabla sin política ⇒ no ofrecible · centinela `capable` con `columnRules` ⇒ no ofrecible · centinela `capable` **sin** `columnRules` ⇒ ofrecible (la condición (c) es condicional) · `columnRules` presentes con centinela `incapable` ⇒ **no ofrecible** con motivo «reglas de columna: inferencia por predicado bajo SQL libre (C3)» — default de P-7, sin flag ni env; relajarlo es un cambio de código con decisión de César · centinela ausente con `columnRules` ⇒ no ofrecible (`uninstrumented`) · `fn_my_permissions` lanza ⇒ no ofrecible · re-set tras read_only **no** lanza ⇒ no ofrecible · todo verde ⇒ ofrecible con `verificadoEn`.
...
- **Tests** (`tests/consola-handler.test.ts`, con `http` in-proc como `tests/admin-handler.test.ts`): sin scope ⇒ 403 en las seis rutas, **sin** revelar `enabled` · scope + apagada ⇒ 503 · CSRF inválido ⇒ 403 · ref no ofrecible ⇒ 404 · segunda ejecución de la misma identidad ⇒ 409 y la primera sigue · segunda ejecución de **otra** identidad con `maxConcurrentes=1` ⇒ 503 y la primera sigue; al resolverse la primera, la segunda entra · `cancelar` aborta el signal · standby ⇒ 409 en POST · el `actor` del log es el email del gate aunque el body traiga `actor: 'otro'`.
...
- **Fix**: bandeja con Facetas **Conector** (select), **Esquema** (árbol tablas→columnas, clic inserta el identificador `[sch].[tbl]`), **Límites** (timeout/filas vigentes, leídos de `/contrato`), **Descargar** (CSV · JSON · XLSX), **Historial**; editor `<textarea>` monoespaciado con `Ctrl+Enter`, «Cargar .sql»; resultset con pestañas por recordset, paginación client-side de 100 filas, tiempo, filas, aviso de truncado, error del motor en bloque `<pre>`; aviso fijo: «Esta consola muestra lo que Mira mostraría a ti…». Tema claro/oscuro por los tokens existentes.
...
- **No confundir (c) con C3.** (c) es herencia de `UNMASK` y está resuelta por doctrina (grantee nombrado); C3 es inferencia por predicado y es un límite de DDM. Un C3 rojo **no** manda a tocar la doctrina de `UNMASK`; un (c) verde **no** cierra C3.
- **No subir `VERGIS_CONSOLA_MAX_CONCURRENTES` por encima de 1** en tests vivos ni en docs como «recomendado»: el default es decisión de César.
...
**C3 corre primero, y es gratis:** no necesita ventana de Fabric, ni SP nuevo, ni nada de I1–I3 — solo la fixture existente (`dbo.areas` con `rut` enmascarado) y un login sin `UNMASK`. Es lo primero que el ejecutor corre al abrir el worktree, porque su resultado fija el veredicto de (c) para Conectores con `columnRules` (P-7) antes de escribir el gate.
...
| **C3** | **Inferencia por predicado bajo DDM** (arranca gratis en Docker, sin ventana de Fabric): bajo `consola_lab` (sin `UNMASK`, lee `xxxx`), correr `SELECT COUNT(*) FROM dbo.areas WHERE rut BETWEEN a AND b`, `… WHERE rut LIKE '33.%'`, `… WHERE rut = '<valor real>'` y `… ORDER BY rut` variando rangos y comparando contra el valor real conocido de la fixture. **Control positivo:** las mismas consultas bajo un login con `UNMASK` discriminan (si no, el instrumento no mide). **Control de premisa:** `SELECT rut` devuelve `xxxx` | alguna forma acota el valor real (el `COUNT` cambia con el rango, el `=` acierta, el `ORDER BY` ordena por el valor real) | **Resultado esperado según la documentación de Microsoft: refuta** — DDM enmascara la proyección, no el predicado. Si refuta: (c) para Conectores con `columnRules` queda **«no ofrecible, punto»** (default de P-7) y `docs/consola-sql.md` lo dice con la corrida citada. Si **no** refuta en Docker: se repite en Fabric antes de relajar nada (Docker solo refuta para la familia). **En ningún caso C3 toca la doctrina de `UNMASK`** |
| **C10** | 20 ejecuciones consecutivas de `SELECT 1` por I2 (conexión dedicada) | p95 > 3 s | Se registra p50/p95 en el PR; no es gate, es el costo declarado |
...
Las mismas C1, C2, C4, C6, C7 y C8 con un **segundo SP con rol `Viewer`** en el workspace del lab (crearlo es parte de la corrida y está documentado en `scripts/README-fabric-lab.md`). **C6 en Fabric es la medición directa de (c):** con el warehouse del lab bajo la doctrina vigente —`GRANT UNMASK ON [dbo].[areas]([rut]) TO [vergis-lab-serving-sp]`, el grantee nombrado— el SP de consola debe leer `xxxx` en base y vista, y el centinela bajo él debe reportar `incapable`. Lo medido el 2026-09-21 (`4d9b5e8`) es que el `GRANT` al principal nombrado acepta y surte efecto para **ese** principal, con control negativo por `REVOKE`; que un **segundo** principal no lo herede es consecuencia de que el grantee sea nombrado y no `public`, y **se mide acá con el SP de consola** en vez de darse por sentada. Si C3 refutó en Docker, en Fabric se repite solo como corroboración, no como gate (el veredicto fail-closed ya está tomado). **Sin C2 verde en Fabric la Fase 1 no se mergea**: es la afirmación de mecanismo que sostiene todo el plano de fila, y el arnés de Docker solo refuta para la familia, no afirma para Fabric. Registrar en el PR qué se midió en Docker, qué en Fabric y qué **no** se midió (p. ej. SQL endpoint de lakehouse, si el lab no lo tiene).
...
- ☐ Tope de filas por streaming con cancelación efectiva (C7); timeout con cancelación efectiva (C8); 1 en vuelo por identidad; **una consulta a la vez en el nodo** (`maxConcurrentes=1` por default, test de 503 a la segunda identidad); cancelación explícita funciona.
...
- ☐ **C3 corrido en Docker antes de I3**, con control positivo y de premisa, y su veredicto citado en `docs/consola-sql.md`; si refutó, un Conector con `columnRules` no es ofrecible y `/contrato` lo dice con ese motivo (P-7).
- ☐ `npm run typecheck` y `npm test` verdes; C1–C10 corridas y registradas; **C2 verde en Fabric**; C6 en Fabric con el grantee nombrado y el SP de consola `incapable`.
...
| Instancia que todavía concede `GRANT UNMASK … TO [public]` y tiene reglas de columna | Conector no ofrecible por (c), motivo «principal con UNMASK y hay reglas de columna» | Es el diseño, y el remedio ya es doctrina: `REVOKE … FROM [public]` + `GRANT … TO [<principal de serving>]` (`docs/gobierno-permisos.md`; el `REVOKE` por columna se verificó inmediato en el plano de datos). Cambio al terreno ⇒ práctica `c` del lab |
| C3 refuta (DDM no protege del predicado) | Ningún Conector con `columnRules` es ofrecible | Es el default (P-7) y hoy cuesta cero en la instancia de referencia (0 de 8 con reglas de columna, informado por `lab/work/255/03`). Relajarlo es decisión de César, con la corrida a la vista |
| La Consola compite por la capacidad F2 con los PI en servicio | Un PI lento mientras alguien consulta; **hoy no hay instrumento que lo distinga de una consulta pesada** | `maxConcurrentes=1` + timeout + tope de filas acotan el daño; **abrirla en la instancia exige antes el instrumento de fallos por capacidad** (§17, condición de César). Reversión: `VERGIS_CONSOLA_ENABLED=0` (env ⇒ ventana 17 bis) |
| Costo del login por ejecución | C10 alto | Aceptable para una consola; si no, un pool **por identidad** con `read_only` es posible **solo** si las claves inyectadas no cambian entre requests de la misma persona — no se hace en v1 |
...
| ~~**P-2**~~ | ¿La Consola necesita un permiso propio de superficie? | **CERRADA por César el 2026-09-21: no se restringe.** Restringir no protege el dato (lo hace el gate) y tampoco es control de acceso: la política es la autoridad, no el PI. Queda una **verificación** previa —las tablas `grant: all` por Conector, abiertas pensando en otro lector— y la línea de costo de capacidad, que es suya porque se paga | Nada del gate |
| ~~**P-3**~~ | ¿Export XLSX, contra la decisión de #61? | **DESPACHADA por César el 2026-09-21: opción B** — «escritor XLSX propio, client-side, sin dependencia». Su decisión de #61 (2026-07-13) descartaba el `.xlsx` por el **costo** —«la primera dependencia gorda del producto y un endpoint server-side, sin demanda que lo justifique»—, no por el formato; esta vía no paga ninguno de los dos, así que la razón de entonces deja de aplicar sin contradecirla. ⚠️ **Marcó la opción que lleva su propia conjetura escrita**: que un escritor mínimo sea barato **no está medido**. El ejecutor lo mide antes de comprometer alcance; si no resulta barato vuelve al despacho, y **no** se convierte en dependencia por inercia (el presupuesto del ADR-001 sigue en pie) | Export XLSX de una hoja, sin estilos ni fórmulas, junto a CSV y JSON |
| ~~**P-4**~~ | Doctrina de `UNMASK` en instancias con Consola | **CERRADA por MEDICIÓN el 2026-09-21** (commits `4d9b5e8`, `84b8df7`; `docs/gobierno-permisos.md`; ficha `lab/work/255/03`): `GRANT UNMASK ON [tabla]([col]) TO [<principal nombrado>]` **acepta y surte efecto sin `CREATE USER`**, con premisa y control negativo. La doctrina nombra al principal de serving; `public` es último recurso. El principal de consola no hereda `UNMASK` ⇒ (c) pasa ⇒ el recorte desaparece. Sin medir, con esas palabras: `appId` como grantee, y el SKU de un cliente | Nada |
| **P-7** | Si C3 refuta (DDM permite inferir por predicado): ¿un Conector con `columnRules` se ofrece **declarándolo** en la superficie, o **no se ofrece**? | **Default del diseño: no se ofrece** — es la única lectura consistente con «no ofrecer con advertencia» (§6) y hoy cuesta cero (0 de 8 almacenes con reglas de columna). El ejecutor implementa el default; **relajarlo** (ofrecer con declaración, o exigir que la instancia quite las reglas de columna de ese Conector) es decisión de César con la corrida de C3 a la vista. No bloquea la Fase 1 | Fase 2 solo en warehouses que adquieran reglas de columna |
| ~~**P-5**~~ (sin objeto: colgaba del grupo que no existe) | Nombre del grupo de scope (`consola-sql`) y quién entra en la instancia | El default; miembros los decide César | Fase 2 |
...
5. `VERGIS_CONSOLA_ENABLED=1` en `vergis.env` ⇒ **cambio de env ⇒ ventana 17 bis**. `VERGIS_CONSOLA_MAX_CONCURRENTES` **no se declara** (queda en el default `1`): «una consulta a la vez» es decisión de César para esta instancia (ficha `lab/work/255/02`).
...
7. Si un warehouse tiene reglas de columna y todavía concede `UNMASK` a `public`: migrar al grantee nombrado según la doctrina (`REVOKE … FROM [public]`; `GRANT … TO [<SP de serving>]`), cambio al terreno ⇒ práctica `c`; el gate (c) lo verifica solo. Si además C3 refutó: ese Conector **no se ofrece** hasta que César despache P-7.
8. 🙋 **Condición de César que ata la aceptación (ficha `lab/work/255/02`): antes de abrir la Consola en la instancia tiene que existir el instrumento que mide los fallos por capacidad de la F2** — hoy no existe, y es un hueco de la operación actual, no solo de la Consola. Lo que hay: `queryinsights` en cada warehouse (6 vistas medidas en `wh_finanzas`, sin usar); lo que falta: el dato de estrangulamiento **a nivel de capacidad** (API de métricas de Fabric) y la telemetría de render de PI (`vergis#328`). Dos cautelas de diseño que viajan con él: **(1)** antes de creerle, forzar un caso y ver que aparece — un panel verde por no saber leer el dato es peor que ninguno; **(2)** «lento porque la consulta es pesada» y «lento porque la capacidad está estrangulada» se ven igual desde afuera, y solo el dato de capacidad los discrimina. **Sin este instrumento, la aceptación no se ejecuta** — es trabajo aparte (no de la Fase 1) y su ausencia bloquea el paso 5.
9. Verificación previa a abrir (P-2): listar las tablas `grant: all` por Conector ofrecible y decirle a César si alguna quedó abierta por comodidad y no por decisión.

---

## ¿Qué queda fuera de alcance de v1?

Gestión de Conectores, usuarios, roles o políticas desde la Consola · escritura (P-1) · ClickHouse (P-6) · **export server-side** (el XLSX de P-3 es client-side por decisión) · editor con resaltado/autocompletado (sin dependencia no hay uno decente; un `<textarea>` con árbol de esquema clicable cubre el MVP) · favoritos con nombre (v1.1, tabla en `GovernanceStore`) · `GO` y bloques PL/SQL · gráficos, diseñador visual, edición inline · convertir la Consola en PI o en proto-Botlet · export server-side · XLSX (P-3).
